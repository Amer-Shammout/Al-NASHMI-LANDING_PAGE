  ...
  const contactList = CONTACT_DATA.contact_info.map((contact, index) => {
    ...
    return (
      <button
        ...
        className="flex items-center gap-4 group cursor-pointer p-4 rounded-xl bg-white/10 backdrop-blur-lg hover:bg-white/20 transition-all duration-300 hover:shadow-lg border border-white/60 hover:border-(--color-primary)/30 text-right focus:outline-none focus:ring-2 focus:ring-(--color-primary)/50 flex-1 min-w-[280px] max-w-[400px]"
      >
        {/* الأيقونة */}
        <div className="flex items-center justify-center w-12 h-12 bg-white/20 backdrop-blur-lg rounded-lg group-hover:bg-(--color-primary)/10 transition-all duration-300 shadow-sm">
          <IconComponent className="text-(--color-primary) text-lg group-hover:scale-110 transition-transform duration-300" />
        </div>

        {/* الخط الفاصل العمودي */}
        <div className="w-px h-8 bg-linear-to-b from-transparent via-gray-300 to-transparent group-hover:via-(--color-primary) transition-all duration-300"></div>
